...
class NewsSlider extends \yii\db\ActiveRecord
{
    ...
    public function getImage($for = '_form')
    {
        ...
        if($for =='_form') {
            return $this->image ? '<img style="width:100%; height:200px; border-radius:10%;" src="/'.$adminka.'/uploads/news_slider/' . $this->image .'">' : '<img style="width:100%; height:200px; border-radius:10%;" src="/'.$adminka.'/uploads/noimg.jpg">';
        }
        if($for == '_columns') {
           return $this->image  ? '<img style="width:90px; border-radius:10%;" src="/'.$adminka.'/uploads/news_slider/' . $this->image .' ">' : '<img style="width:60px;" src="/'.$adminka.'/uploads/noimg.jpg">';
        }
        if($for == 'main_page') {
