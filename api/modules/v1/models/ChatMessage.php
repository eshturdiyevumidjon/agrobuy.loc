...
class ChatMessage extends \yii\db\ActiveRecord
{
    ...
    public function getAdsFiles()
    {
        ...
        if (!file_exists($_SERVER['DOCUMENT_ROOT'] . '/backend/web/uploads/chat/' . $this->file)) {
            $path = $siteName . '/backend/web/img/no-logo.png';
        } else {
            $path = $siteName . '/backend/web/uploads/chat/' . $this->file;
        }
        return $path;
    }
    ...
    {   
        ...
        $card = $v1.'chat/' . 'profile?id=';

        $identity = Yii::$app->user->identity;
        $date = null; $show = true; $result = []; $type = "";
        ...
        foreach ($messages as $message) {
            ...
            if($message->user_id == $identity->id) {
                $creator = 'me';
                $link = $card.$identity->id;
            }
            else {
                $creator = 'you';
                $link = $card.$message->user_id;
            }

            if($message->file != null) { 
                $msg = $message->getAdsFiles();  $type = "file"; 
            }
            else { $msg = $message->message; $type = "text" ;}
            $result [] = [
                'type'=>$type,
                'creator' => $creator,
                'class' => $class,
